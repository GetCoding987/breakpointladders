...
export async function sendNotificationEmail({ toEmail, fullName, type, title, body }) {
	...
	if (!toEmail) return { skipped: true, reason: 'no email on file for user' };

	if (!process.env.RESEND_API_KEY) {
		console.warn('RESEND_API_KEY not set — skipping email send', { toEmail, type });
	...
	const { data, error } = await resend.emails.send({
		from: process.env.RESEND_FROM_ADDRESS || 'Break Point Ladders <onboarding@resend.dev>',
		to: toEmail,
		subject,
		html,
	});
