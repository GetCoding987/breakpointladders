...
function buildEmailHtml({ fullName, title, body, type }) {
	...
	const ctaUrl = isMembershipNotice ? `${APP_URL}/join` : isForfeitLoss ? `${APP_URL}/profile` : APP_URL;
	const ctaLabel = isMembershipNotice ? 'Renew Membership' : isForfeitLoss ? 'Go to My Profile' : 'View on Break Point';
...
export async function sendNewUserAlert({ fullName, email }) {
	...
	const html =
		`<div style="font-family: Arial, Helvetica, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; color: #333333;">` +
		`<p style="margin: 0 0 16px 0; font-size: 16px;">A new user just created an account:</p>` +
		`<p style="margin: 0 0 8px 0; font-size: 16px;"><strong>Name:</strong> ${escapeHtml(name)}</p>` +
		`<p style="margin: 0; font-size: 16px;"><strong>Email:</strong> ${escapeHtml(email || 'unknown')}</p>` +
		`</div>`;
