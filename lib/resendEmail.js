...
const LOGO_URL = `${APP_URL}/BPW_OptionA_NormalA_UseOnDark_600x200_transparent.png`;

const EMAILED_TYPES = [
	'challenge_received',
	'challenge_accepted',
	'challenge_declined',
	'challenge_forfeit_won',
	'challenge_forfeit_lost',
	'challenge_expired',
	'score_submitted',
	'score_confirmed',
	'score_disputed',
...
function buildEmailText({ fullName, body }) {
	const firstName = fullName ? fullName.split(' ')[0] : 'Player';
	return `Hi ${firstName},\n\n${body || ''}\n\n${APP_URL}\n\nThe Break Point Ladder Team`;
}
...
function buildEmailHtml({ fullName, title, body, type }) {
	...
	const isForfeitLoss = type === 'challenge_forfeit_lost';
	const isNewUserAlert = type === 'new_user_alert';
	const ctaUrl = isMembershipNotice ? `${APP_URL}/join` : isForfeitLoss ? `${APP_URL}/profile` : isNewUserAlert ? `${APP_URL}/admin` : APP_URL;
	const ctaLabel = isMembershipNotice ? 'Renew Membership' : isForfeitLoss ? 'Go to My Profile' : isNewUserAlert ? 'Go to Admin Panel' : 'View on Break Point';
...
export async function sendNotificationEmail({ toEmail, fullName, type, title, body }) {
	...
	const text = buildEmailText({ fullName, body });
	...
	const { data, error } = await resend.emails.send({
		...
		html,
		text,
		reply_to: 'manager@breakpointladders.com',
		headers: { 'List-Unsubscribe': '<mailto:manager@breakpointladders.com>' },
	});
...
export async function sendNewUserAlert({ fullName, email }) {
	if (!process.env.RESEND_API_KEY) {
		throw new Error('RESEND_API_KEY not configured');
	}
	...
	const subject = `New user joined: ${name}`.replace(/[\r\n]/g, '');
	const body = `A new user just created an account:\n\nName: ${name}\nEmail: ${email || 'unknown'}`;
	const html = buildEmailHtml({ fullName: 'Manager', title: subject, body, type: 'new_user_alert' });
	const text = buildEmailText({ fullName: 'Manager', body });

	const { data, error } = await resend.emails.send({
		from: process.env.RESEND_FROM_ADDRESS || 'Break Point Ladders <noreply@breakpointladders.com>',
		to: 'manager@breakpointladders.com',
		subject,
		html,
		text,
	});
